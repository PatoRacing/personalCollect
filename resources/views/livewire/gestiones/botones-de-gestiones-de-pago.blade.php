...
<div class="grid grid-cols-1 md:grid-cols-{{ count($acciones) }} justify-center gap-1 mt-2">
    @if (!empty($acciones)) {{-- Verifica si hay acciones disponibles --}}
        ...
        @endforeach
    @else
        {{-- Mensaje opcional si no hay acciones --}}
        <p class="p-2 text-sm text-center bg-gray-400 text-white">No hay acciones disponibles para esta cuota.</p>
    @endif
</div>
