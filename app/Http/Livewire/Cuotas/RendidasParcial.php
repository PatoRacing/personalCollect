...
class RendidasParcial extends Component
{
    use WithPagination;
    ...
    protected $listeners = ['terminosBusquedaDeCuotasRendidasParcial'=> 'buscarCuotasRendidasParcial'];
    ...
    {
        $cuotasRendidasParcial = Pago::query();
        //Busqueda por deudor
        if ($this->deudor) {
            $deudorId = Deudor::where('nombre', 'LIKE', "%" . $this->deudor . "%")
                                ...
                                ->first();
            if ($deudorId) {
                $cuotasRendidasParcial->whereHas('acuerdo.propuesta', function($subquery) use ($deudorId) {
        ...
        if ($this->nro_doc) {
            $cuotasRendidasParcial->whereHas('acuerdo.propuesta.operacionId', function($subquery) {
        ...
        // Busqueda por mes
        if ($this->mes) {
            $cuotasRendidasParcial->whereMonth('vencimiento_cuota', $this->mes);
        }
        //Busqueda por vencimiento
        if ($this->estado) {
            $hoy = Carbon::today();
            if ($this->estado == 'activo') {
                $cuotasRendidasParcial->where('vencimiento_cuota', '>=', $hoy);
            } else if ($this->estado == 'vencido') {
                $cuotasRendidasParcial->where('vencimiento_cuota', '<', $hoy);
            }
        }
        //Vista sin busqueda
        $cuotasRendidasParcial = $cuotasRendidasParcial->where('pagos.estado', 4)  
                                    ...
                                    ->paginate(30);
        $cuotasRendidasParcialTotales = $cuotasRendidasParcial->total();

        return view('livewire.cuotas.rendidas-parcial',[
            'cuotasRendidasParcial'=>$cuotasRendidasParcial,
            'cuotasRendidasParcialTotales'=>$cuotasRendidasParcialTotales,
        ]);
    }
}
